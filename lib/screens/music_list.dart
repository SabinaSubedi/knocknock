...
class _MusicListScreenState extends State<MusicListScreen> {
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState != ConnectionState.done) {
            ...
            snapshot.data['items'].forEach((key, item) => files.add(
                FirebaseFile(
                    name: key.replaceAll('.mp3', ''), path: item['path'])));
                  ...
                  return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                            ...
                            trailing: IconButton(
                              icon:
                                  Selector<MusicListNotifier, AudioPlayerState>(
                                      selector: (context, musicList) =>
                                          musicList.audioPlayerState,
                                      builder:
                                          (context, audioPlayerState, child) {
                                        int musicIndex =
                                            Provider.of<MusicListNotifier>(
                                                    context,
                                                    listen: false)
                                                .currentMusicIndex;
                                        return FaIcon(audioPlayerState ==
  ...
  void _playAudio(String path, index) async {
    ...
    AudioPlayerState audioPlayerState =
        Provider.of<MusicListNotifier>(context, listen: false).audioPlayerState;
    Provider.of<MusicListNotifier>(context, listen: false)
