...
class _ChatScreenState extends State<ChatScreen> {
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Message(
                          ...
                          time: dateFormat.format(DateTime.fromMillisecondsSinceEpoch(chats[index]['time']).toLocal())
                        );
                      },
                    );
  ...
  Stream<QuerySnapshot> _getChats(BuildContext context) {
    FirebaseUser you = Provider.of<UserNotifier>(context, listen: false).user;
    String key = widget.user.uid + you.uid;
    return Firestore.instance.collection('chats').where('users', arrayContains: key ).orderBy('time').snapshots();
  }

  void _sendMessage(BuildContext context, User other, String message) async {
    var time = new DateTime.now().toUtc().millisecondsSinceEpoch;
    try {
      FirebaseUser you = await FirebaseAuth.instance.currentUser();
      ...
      await Firestore.instance.collection('recentChats').document(you.uid).collection('userIds').document(widget.user.uid).setData({
        'time': time
      });
      await Firestore.instance.collection('recentChats').document(widget.user.uid).collection('userIds').document(you.uid).setData({
        'time': time
      });
